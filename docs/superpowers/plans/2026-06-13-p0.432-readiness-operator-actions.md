...
**Architecture:** Keep the stale-readiness primary command unchanged, then carry sanitized sequence row actions into the readiness sequence. Device actions expose the first preserved row action as `saved_next_action` while retaining `command` for the immediate stale-report rerun.
...
Expected: the test fails because `sequence_row["operator_actions"]` is missing, or because `first_action["saved_next_action"]` is missing.
