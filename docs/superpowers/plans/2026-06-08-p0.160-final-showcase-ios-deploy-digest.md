...
- [x] Fast-forward merge `codex/p0.160-final-showcase-ios-deploy-digest` into `main`.
- [x] Re-run the verification set on `main`.
- [x] Remove the P0.160 worktree if verification passes.
...
- Branch: `codex/p0.160-final-showcase-ios-deploy-digest`
- Design commit: `67313a3 docs: design final showcase ios deploy digest`
- Plan commit: `cf0bf49 docs: plan final showcase ios deploy digest`
- Implementation commit: `c08d612 feat: show ios deploy digest in final showcase`
- Merge: fast-forwarded into `main` at `c08d612`.
- Cleanup: removed `/Users/zhexu/Codex/personal-myth-forge/.worktrees/p0.160-final-showcase-ios-deploy-digest` and deleted the merged branch.

Verification on branch:

- `make backend-lint` passed.
- `make backend-test` passed: 473 passed, 1 known Starlette/TestClient deprecation warning.
...
- `swift build --package-path apps/mobile/ios` passed.
- `make visual-regression` passed: 18 artifacts, 0 failed.
