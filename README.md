...
`make final-demo-launch` remains as a compatibility alias for the local command.
...
gate now checks 30 static 390x844 iPhone evidence artifacts covering scan input,
...
resource requirements next input, final demo launch next action, auto backend
URL handoff, final showcase device actions, preflight evidence device actions,
Xcode evidence device actions, configured evidence, resource fill guide, device
evidence, closure packet, final launch visual handoff, and the device blocker
handoff visual.
The device blocker handoff visual remains in the full-showcase index.
...
providers, run Xcode, install to a device, or mutate global machine state.
