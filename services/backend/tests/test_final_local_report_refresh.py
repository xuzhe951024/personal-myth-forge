...
def test_final_local_report_refresh_final_showcase_uses_loaded_settings(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    def fake_final_showcase_report(
        *,
        settings: Settings,
        repo_root: Path,
    ) -> SimpleNamespace:
    ...
    monkeypatch.setattr(
        ...
        fake_final_showcase_report,
    )

    step = next(
        step
        for step in final_local_report_refresh._default_steps()
        if step.id == "final_showcase_readiness"
    )

    report = step.runner(tmp_path)

    assert report["status"] == "partial"
    assert captured["settings"] is configured_settings


def test_final_local_report_refresh_configured_bundle_step_reuses_saved_plan(
    tmp_path: Path,
) -> None:
    ...
    (local_dir / "final-configured-evidence-plan.json").write_text(
        json.dumps(saved_plan),
        encoding="utf-8",
    )
    configured_bundle_step = next(
        step
        for step in final_local_report_refresh._default_steps()
        if step.id == "configured_live_evidence_bundle"
    )

    report = configured_bundle_step.runner(repo_root)
    ...
    assert report["current_blocker"]["classification"] == "consent_required"
...
def test_final_local_report_refresh_writes_safe_reports_without_live_or_global_actions(
    ...
    expected_detail = (
        ...
        "Next device action: DEVELOPMENT_TEAM=YOUR_TEAM_ID PMF_BACKEND_BASE_URL=auto "
...
def test_final_local_report_refresh_operator_actions_prefer_deploy_writer() -> None:
    ...
    assert bundle_action not in actions
