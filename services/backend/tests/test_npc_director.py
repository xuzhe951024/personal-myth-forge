...
def test_local_npc_director_returns_stable_three_npc_reactions() -> None:
    ...
    reactions_by_id = {reaction.npc_id: reaction for reaction in result.reactions}
    for trace in result.agent_traces:
        assert trace.proposed_action in reactions_by_id[trace.npc_id].plan
    assert reactions_by_id["senn"].plan == [
        "circle_artifact",
...
def test_openai_npc_director_replaces_trace_action_outside_reaction_plan() -> None:
    parsed = {
        "reactions": [
            {
                "npc_id": "mara",
        ...
        "agent_traces": [
            {
                ...
                "belief": "The key is a promise.",
                "intention": "welcome the artifact",
                "proposed_action": "order_print_without_approval",
                "rationale": "Unsafe action should not be trusted.",
                "confidence": 0.91,
            },
            {
                "npc_id": "ior",
    ...
    mara_trace = result.agent_traces[0]
    assert mara_trace.npc_id == "mara"
    assert mara_trace.proposed_action == "approach_artifact"
    assert mara_trace.rationale.startswith("Synthesized from")
    assert mara_trace.confidence == 0.5
