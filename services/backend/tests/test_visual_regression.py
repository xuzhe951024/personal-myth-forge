...
def test_default_visual_artifacts_cover_full_showcase_flow() -> None:
    assert [spec.id for spec in DEFAULT_VISUAL_ARTIFACTS] == [
        ...
        "p0.457_demo_script_external_actions",
    ]
    ...
    assert "Xcode Evidence" in required_text
    assert "evidence blocked" in required_text
    assert "Apple SDK license agreement is not accepted." in required_text
